...
data_path = 'G:/A PYTHON NOTEBOOK/Bayesian_main/ooddata/tok_lad_scsr_ahc_new.pkl'
...
# Load data
data_path2 = 'G:/A PYTHON NOTEBOOK/Bayesian_main/ooddata/tok_lad_scsr_ahc.pkl'
...
# all_selected_features = [ 0,  6,  7,  8,  9, 13, 17, 19, 20] washington
# all_selected_features = [ 0,  1,  4,  6,  7,  8,  9, 15]
...
def dataset_split(test_mask_list=None, modify=True):
    ...
    for train_index , test_index in kf.split(new_feature_arr): 
        
        X_train_fold, X_test_fold, y_train_fold, y_test_fold = [],[],[],[]
        X_val_fold, y_val_fold = [], []
        for i in train_index:
            X_train_fold.append(new_feature_arr[i])
            y_train_fold.append(label_arr[i])
        ...
        for i in test_index:
            X_test_fold.append(new_feature_arr[i])
            y_test_fold.append(label_arr[i])
...
for dataset in dataset_list:
    ...
    val1, val2 = get_qhat(model, device, X_val_fold[:,:-2], y_val_fold, alpha1_start=0.05, alpha2_start=0.05)
  # model.load_state_dict(torch.load('best_model.pth'))
    ...
    test_mask_list = np.zeros_like(common_mask)
